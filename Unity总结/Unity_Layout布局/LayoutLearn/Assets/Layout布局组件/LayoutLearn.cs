 ...
 *              注意:UI元素自带的隐藏Layout Element，可以通过UI元素的属性 min prefer flexible 获取
 *              3、API：
 *                      minHeight、minWidth、preferredHeight、preferredWidth、flexibleHeight、flexibleWidth
 *                      ignoreLayout：忽略本组件
 *                      layoutPriority：优先级(大于等于0采用本组件，小于0采用原生隐藏组件)、一般1会优先于原生的Image、Text、LayoutGroup
 *              
 ...
 *            4、参考点 pivot
 *                  当改变RectTransform的Size时，pivot 的位置固定，然后根据 pivot 设置方向和大小
*                   比如pivot在center，则朝所有方向等比例适配

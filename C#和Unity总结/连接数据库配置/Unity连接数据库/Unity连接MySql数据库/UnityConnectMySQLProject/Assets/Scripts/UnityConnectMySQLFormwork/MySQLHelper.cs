...
namespace Unity_MySQL_Formwork
{
    public enum Type
    {
    ...
    public class MySQLHelper
    {
        ...
        public static void OpenDataBase(string host, int port, string databaseName, string userName, string passwd)
        {
            ...
            try
            {
                ...
                string str = String.Format("Host = {0};port ={1};UID ={2};PWD={3};Database ={4};", host, port, userName,
                    passwd, databaseName);
                try
                {
                ...
                catch (MySqlException)
                {
                    ...
                    try
                    {
                        ...
                        string tmpstr = String.Format("Host = {0};port ={1};UID ={2};PWD={3};", host, port, userName,
                            passwd);
        ...
        private static MySqlDataReader ExecuteQuery(string queryString, string message = null)
        {
            ...
            {
                Debug.Log(message + "失败" + e.Message);
            }
        ...
        public static void ShowData(MySqlDataReader reader)
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    str += "第" + i + "列:" + "(列名:" + reader.GetName(i) + "  类型:" + reader.GetDataTypeName(i) + ") " +
                           reader.GetValue(i) + " | ";
                }

                int temp = reader.FieldCount - 1;
                str += "第 " + temp + " 列:" + "(列名:" + reader.GetName(temp) + "  类型:" + reader.GetDataTypeName(temp) +
        ...
        public static MySqlDataReader CreateTable(string tableName, string[] colNames, string[] colTypes,
            string[] colLength, string[] colState)
        {
            ...
            {
                Debug.Log("colNames colTypes colLength colState length not equail");
                return null;
            }
            ...
            dbCommand.CommandText =
                string.Format(
                    ...
                    tableName); /*"select COUNT(*) from information_schema.TABLES where TABLE_SCHEMA = \'" + _CurrentdataBaseName + "\'  AND table_name = \'" + tableName + "\'";*/
            ...
            else
            {
                ...
                {
                    queryString += colNames[i] + " " + colTypes[i] + "(" + colLength[i] + ")" + " DEFAULT " +
                                   colState[i] + ",";
                }
        ...
        public static int DeleteDataOR(string tableName, string[] colNames, string[] colValues, string[] operations)
        {
            ...
            string queryString = "DELETE FROM " + tableName + " WHERE " + colNames[0] + operations[0] + "'" +
                                 colValues[0] + "'";
        ...
        public static int UpdateDataOR(string tableName, string[] colNames, string[] colValues, string[] key,
            string[] value, string[] operations)
        {
            ...
            {
                throw new Exception(
                    "key.Length!=value.Length || operations.Length!=key.Length || operations.Length!=value.Length");
            }
        ...
        /// <param name="value">依据的关键字对应的值</param>
        /// <param name="operation">依据运算符：= , < , >,...</param>
        public static int UpdateDataAND(string tableName, string[] colNames, string[] colValues, string[] key,
            string[] value, string[] operations)
        {
            //当字段名称和字段数值不对应时引发异常
            if (colNames.Length != colValues.Length)
        ...
        public static MySqlDataReader SelectDataAND(string tableName, string[] keys, string[] colNames,
            string[] colValues, string[] operations)
        {
            //当字段名称和字段数值不对应时引发异常
            if (colNames.Length != colValues.Length || operations.Length != colNames.Length ||
                operations.Length != colValues.Length)
        ...
        /// <param name="colValues">依据的字段名对应的数据</param>
        /// <param name="operations">依据的运算符：= , < , >,...</param>
        public static MySqlDataReader SelectDataOR(string tableName, string[] keys, string[] colNames,
            string[] colValues, string[] operations)
        {
            //当字段名称和字段数值不对应时引发异常
            if (colNames.Length != colValues.Length || operations.Length != colNames.Length ||

...
namespace UpFileStreamTest
{
    ...
    {
        public const int BUFFER_COUNT = 1024000;
        ...
        /// <param name="cutNum">分成几次传输</param>
        public static void CopyBigFile(string localFilePath, string uploadFilePath,int cutNum)
        {
            if(!File.Exists(localFilePath))return;
            FileInfo info = new FileInfo(localFilePath);
            long fileLength = info.Length;
            int perFileLength = (int)fileLength / cutNum;
            int restLength = (int) fileLength % cutNum;
            for (int i = 0; i <= cutNum; i++)
            {
                int startPostion = i * perFileLength;
                long currentCount = fileLength - startPostion < perFileLength
                    ? restLength
                    : perFileLength;
                UpLoadFileFromLocal(localFilePath, uploadFilePath, startPostion, (int)currentCount);
            }
            Console.WriteLine("完成");
        }

        private static void UpLoadFileFromLocal(string localFilePath,string uploadFilePath,int startPosition,int currentCount)
        {
            if(currentCount==0)return;
            int tempReadCount = 0;
            int tempBuffer = BUFFER_COUNT;
            ...
            using (FileStream fileStream = new FileStream(localFilePath, FileMode.Open))
            {
                ...
                {
                    int writeStartPosition = startPosition + tempReadCount;
        ...
        private static void WriteToServer(string filePath,int startPosition,byte[] bytesArr)
        {
